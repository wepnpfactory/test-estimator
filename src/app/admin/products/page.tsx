...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    DRAFT: "bg-zinc-100 text-zinc-600 ring-zinc-500/20 dark:bg-zinc-800 dark:text-zinc-300 dark:ring-zinc-500/30",
    ARCHIVED:
    ...
    <div className="space-y-6">
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center rounded-xl border border-dashed border-zinc-300 bg-white/50 px-6 py-16 text-center dark:border-zinc-700 dark:bg-zinc-900/40">
          ...
          <div className="mt-1 text-xs text-zinc-500">첫 상품을 연결해 견적 엔진을 활성화하세요.</div>
          <Link
            href="/admin/products/new"
        ...
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none dark:ring-1 dark:ring-white/5">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-200/70 dark:divide-zinc-800">
              {products.map((p) => (
                <tr key={p.id} className="transition-colors hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40">
                  <td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">{p.name}</td>
                  <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{p.mall.name}</td>
                  <td className="px-4 py-3 tabular-nums text-zinc-600 dark:text-zinc-400">
                    {p.cafe24ProductNo ?? "—"}
                  </td>
                  <td className="px-4 py-3 tabular-nums text-zinc-600 dark:text-zinc-400">{p._count.optionGroups}</td>
