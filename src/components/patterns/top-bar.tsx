...
function TopBar({ logo, subtitle, actions, className, ...props }: TopBarProps) {
  return (
    <header data-slot="top-bar" className={cn("px-6 pt-8 pb-6", className)} {...props}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">{logo}</div>
        {actions && <div className="flex items-center gap-3">{actions}</div>}
      </div>
      {subtitle && <p className="text-[13px] text-text-tertiary font-medium mt-3">{subtitle}</p>}
    </header>
  );
}
...
function TopBarAction({ badge, className, children, ...props }: TopBarActionProps) {
  return (
    <button
    ...
    >
      {children}
      {badge && <span className="absolute top-1 right-1 size-1.5 bg-alert-badge rounded-full" />}
    </button>
  );
}
