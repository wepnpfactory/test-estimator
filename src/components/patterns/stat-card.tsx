...
function StatCard({ icon: Icon, label, value, unit, trend, children, className, ...props }: StatCardProps) {
  return (
    <div
      data-slot="stat-card"
      className={cn("bg-card rounded-2xl p-6 shadow-[var(--shadow-card)]", className)}
      {...props}
    >
      ...
      {trend && (
        <div className="flex items-center gap-1">
          <span className={cn("text-[13px] font-bold", trend.direction === "up" ? "text-success" : "text-destructive")}>
            {trend.value}
          </span>
          {trend.direction === "up" ? (
