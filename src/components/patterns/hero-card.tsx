...
function HeroCard({
  ...
  return (
    <div
      data-slot="hero-card"
      className={cn("mx-6 rounded-2xl bg-card p-8 shadow-(--shadow-card) relative overflow-hidden", className)}
      {...props}
    >
      {backgroundElement && <div className="absolute inset-0 opacity-[0.15]">{backgroundElement}</div>}
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[12px] text-text-secondary font-medium uppercase tracking-[0.05em]">{label}</p>
        </div>
        ...
        {trend && (
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1">
              <span
                className={cn("text-[15px] font-bold", trend.direction === "up" ? "text-success" : "text-destructive")}
              >
                {trend.value}
              </span>
              {trend.direction === "up" ? (
            ...
            {trend.label && <span className="text-[13px] text-text-tertiary font-medium">{trend.label}</span>}
          </div>
        )}
      </div>
    </div>
