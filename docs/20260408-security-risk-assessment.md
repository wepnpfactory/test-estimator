...
## 운영 모델 (확인)
- **겉보기 상품(Product) ↔ 동적 상품(DynamicProduct)** 은 1:N. 한 겉보기 상품에 여러 옵션 조합이 모두 별개 동적 상품으로 등록됨.
...
### H-E (보상 트랜잭션) — 영향 재평가
- 한 겉보기 상품에 N개 동적 상품이 정상적으로 누적되는 모델이므로, **orphan과 정상 누적이 외형상 구분 불가**. cleanup 배치가 잘못 동작하면 정상 IN_CART 상품을 지울 수 있음.
- 권고:
